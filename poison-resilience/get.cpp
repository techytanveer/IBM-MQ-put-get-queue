...
{
    MQHCONN hConn;
    MQHOBJ hObj;    // first Object Handle to point to Message Queue
    MQHOBJ hObjDLQ; // second Object Handle to point to Dead-Letter Queue
    ...
    MQOD objDesc = {MQOD_DEFAULT}; // Message Queue
    MQOD odDLQ = {MQOD_DEFAULT};   // Dead-Letter Queue

    strncpy(objDesc.ObjectName, "TEST.QUEUE", MQ_Q_NAME_LENGTH);          // Message Queue
    strncpy(odDLQ.ObjectName, "DEV.DEAD.LETTER.QUEUE", MQ_Q_NAME_LENGTH); // Dead-Letter Queue

    MQLONG openOptions = MQOO_INPUT_AS_Q_DEF | MQOO_FAIL_IF_QUIESCING;
    MQLONG openOptionsDLQ = MQOO_OUTPUT | MQOO_FAIL_IF_QUIESCING;

    MQOPEN(hConn, &objDesc, openOptions, &hObj, &compCode, &reason);     // MQ
    MQOPEN(hConn, &odDLQ, openOptionsDLQ, &hObjDLQ, &compCode, &reason); // Dead
    ...
    if (compCode == MQCC_OK)
    {
        ...
        if (compCode == MQCC_OK)
        {
            ...
            // Simulate a processing error for specific messages
            if (strstr(buffer, "POISON") != NULL)
            {
                // SPRINT 2 - RESILIENCE CHECK ----
                std::cout << "POISON DETECTED - Rolling back... : " << buffer << std::endl;

                MQPUT(hConn, hObjDLQ, &md, &pmo, (MQLONG)strlen(buffer), (void *)buffer, &compCode, &reason);

                // MQCMIT(hConn, &compCode, &reason);

                // MQBACK(hConn, &compCode, &reason);
                // return 0; // The while loop will immediately try to GET this again
            }
            else if (strstr(buffer, "PROCESS") != NULL)
            {
                // 5. syncpointing section
                std::cout << "FAIR MESSAGE - Processing... : " << buffer << std::endl;
                MQCMIT(hConn, &compCode, &reason);
            }
